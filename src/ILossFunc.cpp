...
float ILossFunc::calculate_avg_loss(Network &network, std::shared_ptr<Matrix> input, std::shared_ptr<Matrix> expected) {
  std::shared_ptr<Matrix> loss_n = this->calculate_loss(network, input, expected);
