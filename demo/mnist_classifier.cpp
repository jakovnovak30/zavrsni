...
public:
  MnistClassifier() {
    ...
    // this->layers[1] = std::make_unique<Linear>(128, 64, true);
    // this->layers[2] = std::make_unique<Linear>(64, 10, true);
  }
...
int main() {
  ...
  std::unique_ptr<Dataloader> train_loader = std::make_unique<Dataloader>(*trainset, 20);
  std::unique_ptr<Dataloader> test_loader = std::make_unique<Dataloader>(*trainset, 20);

  std::unique_ptr<MnistClassifier> classifier = std::make_unique<MnistClassifier>();
  std::shared_ptr<IOptimizer> sgd_optim = std::make_shared<SGD>(0.02f);

  while(train_loader->hasNext()) {
    auto [input, labels] = train_loader->nextBatch();
    std::cout << "DEBUG-labels: " << labels.toString() << std::endl;
    ...
    autograd::visualize(*loss, "/tmp/image", true);
